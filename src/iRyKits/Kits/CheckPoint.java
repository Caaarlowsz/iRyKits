...
public class CheckPoint implements Listener, CommandExecutor
{
    ...
    public void Check2(final PlayerInteractEvent event) {
        ...
        if (Arrays.CheckPoint.contains(p.getName())) {
            if (player.getItemInHand().getType().equals((Object)Material.FLOWER_POT_ITEM) && event.getAction() == Action.RIGHT_CLICK_AIR) {
                if (!this.local.containsKey(player.getName())) {
                    player.sendMessage("§4Selecione o local antes de usar!");
                    return;
                }
        ...
        else if (event.getAction() == Action.LEFT_CLICK_BLOCK && block.getType().equals((Object)Material.NETHER_FENCE) && this.local.containsKey(player.getName())) {
