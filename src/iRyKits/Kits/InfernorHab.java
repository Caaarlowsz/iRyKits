...
public class InfernorHab implements Listener
{
        ...
        if (event.getRightClicked() instanceof Player) {
            ...
            if (!InfernorHab.Infer.contains(p)) {
                ...
                if (InfernorHab.fighting.containsKey(p.getName()) || InfernorHab.fighting.containsKey(r.getName())) {
                    ...
                    p.sendMessage("§cVoc\u00ea j\u00e1 esta em combate!");
                    return;
                }
                ...
                if (this.generateGlass) {
                    ...
                    for (int bX = -10; bX <= 10; ++bX) {
                        for (int bZ = -10; bZ <= 10; ++bZ) {
                            for (int bY = -1; bY <= 10; ++bY) {
                                ...
                                if (!Main.areaPvP(r)) {
                                    r.sendMessage("§cVoc\u00ea pode usar sua habilidade somente em PvP");
                                    return;
                                }
                                if (!b.isEmpty()) {
                                    event.setCancelled(true);
                                    p.sendMessage("§cVoc\u00ea n\u00e3o pode criar sua arena aqui");
                                    return;
                                }
                                if (bY == 10) {
                    ...
                    Arrays.InfernoHab.add(p.getName());
                    Arrays.InfernoHab.add(r.getName());
                }
                ...
                this.id1 = Bukkit.getScheduler().scheduleSyncDelayedTask(this.plugin, (Runnable)new Runnable() {
                    ...
                    public void run() {
                        if (InfernorHab.fighting.containsKey(p.getName()) && InfernorHab.fighting.get(p.getName()).equalsIgnoreCase(r.getName()) && InfernorHab.fighting.containsKey(r.getName()) && InfernorHab.fighting.get(r.getName()).equalsIgnoreCase(p.getName())) {
                            ...
                            Arrays.InfernoHab.remove(r.getName());
                            ...
                            p.sendMessage("§cN\u00e3o houve nenhum vencedor, voce foi teleportado para o seu lugar de origem!");
                            r.sendMessage("§cN\u00e3o houve nenhum vencedor, voce foi teleportado para o seu lugar de origem!");
    ...
    public void onPlayerLeft(final PlayerQuitEvent e) {
        ...
        if (InfernorHab.fighting.containsKey(p.getName())) {
            ...
            InfernorHab.fighting.remove(p.getName());
            Arrays.InfernoHab.remove(p.getName());
            Arrays.InfernoHab.remove(t.getName());
            final Location old = this.oldl.get(t.getName());
            t.teleport(old);
            t.sendMessage("§aO jogador §c" + p.getName() + "§a deslogou-se!");
    ...
    public void onDeathInferno(final PlayerDeathEvent e) {
        ...
        if (InfernorHab.fighting.containsKey(p.getName())) {
            ...
            k.teleport(old);
            k.sendMessage("§aVoc\u00ea ganhou a batalha contra §c" + p.getName() + "§a!");
            Bukkit.getScheduler().cancelTask(this.id1);
            Bukkit.getScheduler().cancelTask(this.id2);
            ...
            InfernorHab.fighting.remove(p.getName());
            Arrays.InfernoHab.remove(p.getName());
            Arrays.InfernoHab.remove(k.getName());
            final Location loc = this.localizacao.get(p);
            final List<Location> cuboid = new ArrayList<Location>();
            cuboid.clear();
            for (int bX = -10; bX <= 10; ++bX) {
