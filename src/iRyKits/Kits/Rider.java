...
public class Rider implements Listener, CommandExecutor
{
    ...
    public void onClick888(final InventoryClickEvent event) {
        if (event.getCurrentItem() != null && event.getCurrentItem().getType() == Material.SADDLE && event.getWhoClicked().getVehicle() != null && this.horses.containsValue(event.getWhoClicked().getVehicle())) {
    ...
    public void onInteract4(final PlayerInteractEvent event) {
        ...
        if (event.getAction() == Action.RIGHT_CLICK_AIR && p.getItemInHand().getType() == Material.SADDLE && p.getItemInHand().getItemMeta().getDisplayName().equalsIgnoreCase("§aRider")) {
