...
public class Tower implements Listener, CommandExecutor
{
    ...
    public void onPlace(final PlayerInteractEvent event) {
        ...
        if ((event.getAction() != Action.RIGHT_CLICK_BLOCK && event.getAction() != Action.LEFT_CLICK_BLOCK) || item == null || !Arrays.Tower.contains(p2.getName()) || event.getMaterial() != Material.PORTAL) {
            return;
        }
                ...
                Bukkit.getServer().getScheduler().scheduleSyncDelayedTask((Plugin)this.plugin, (Runnable)new Runnable() {
                    ...
                    public void run() {
                        ...
                        for (int j = (arrayOfPlayer = Bukkit.getOnlinePlayers()).length, i = 0; i < j; ++i) {
                            ...
                            if (b.getTypeId() == 120 && Arrays.Tower.contains(p.getName()) && p != mager.getPlayer() && Tower.this.isEnderable(portal, p.getLocation())) {
    ...
    public void stomperApple(final PlayerInteractEvent event) {
        ...
        if (event.getPlayer().getItemInHand().getType() == Material.GOLDEN_APPLE && Arrays.Tower.contains(event.getPlayer().getName())) {
