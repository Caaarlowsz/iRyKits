...
public class Morf implements Listener, CommandExecutor
{
    ...
    public void interact(final PlayerInteractEvent e) {
        ...
        if ((e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) && Arrays.Morf.contains(p.getName()) && p.getItemInHand().getType() == Material.BLAZE_POWDER) {
            if (!Morf.cooldown.containsKey(p.getName()) || Morf.cooldown.get(p.getName()) <= System.currentTimeMillis()) {
                e.setCancelled(true);
                p.updateInventory();
                p.sendMessage(ChatColor.AQUA + "§lMorfando...");
    ...
    public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] Args) {
        if (label.equalsIgnoreCase("morf")) {
            ...
            mf.setItemMeta(kmf);
            BarAPI.setMessage(p, "§6Voc\u00ea Pegou o Kit >> §cMorf", 2);
            p.sendMessage("§6Voc\u00ea Pegou o Kit >> §cMorf");
