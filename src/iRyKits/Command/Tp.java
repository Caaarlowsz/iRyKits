    ...
    public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
        ...
        if (p.hasPermission("iry.teleport")) {
            if (args.length == 0) {
                ...
                final Player target = p.getServer().getPlayer(args[0]);
                final double x3 = args[1].startsWith("~") ? (target.getLocation().getX() + Integer.parseInt(args[0].substring(1))) : Integer.parseInt(args[1]);
                final double y3 = args[2].startsWith("~") ? (target.getLocation().getY() + Integer.parseInt(args[1].substring(1))) : Integer.parseInt(args[2]);
                final double z3 = args[3].startsWith("~") ? (target.getLocation().getZ() + Integer.parseInt(args[2].substring(1))) : Integer.parseInt(args[3]);
                if (x3 > 3.0E7 || y3 > 3.0E7 || z3 > 3.0E7 || x3 < -3.0E7 || y3 < -3.0E7 || z3 < -3.0E7 || target == null) {
                    return true;
                }
                final Location locpos2 = new Location(p.getWorld(), x3, y3, z3, target.getLocation().getYaw(), target.getLocation().getPitch());
                ...
                for (int length4 = (onlinePlayers4 = Bukkit.getOnlinePlayers()).length, l = 0; l < length4; ++l) {
                    ...
                    if (p5.hasPermission("iry.teleport") || p5.isOp()) {
                        p5.sendMessage(ChatColor.GRAY + target.getName() + " foi teleportado para X:" + locpos2.getBlockX() + " Y:" + locpos2.getBlockY() + " Z:" + locpos2.getBlockZ() + " .");
                    }
                }
            }
        }
