...
public class Skit implements Listener, CommandExecutor
{
    ...
    public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
        ...
        if (cmd.getName().equalsIgnoreCase("skit") && (p.hasPermission("iry.skit") || p.isOp())) {
            if (args.length == 0) {
                p.sendMessage(ChatColor.GREEN + "Como criar um skit(kit para eventos):");
            ...
            if (args[0].equalsIgnoreCase("criar")) {
                ...
                p.sendMessage("§aKit " + args[1] + " criado com sucesso!");
                return true;
            }
                ...
                if (!this.kits.containsKey(name) && !this.armor.containsKey(name)) {
                    p.sendMessage("§cKit " + name + " n\u00e3o encontrado!");
                    return true;
                }
                if (this.isInt(args[2])) {
                    ...
                    Bukkit.getServer().broadcastMessage("§aKit " + name + " aplicado para jogadores em um raio de " + numero + " blocos");
                    p.sendMessage("§aKit " + name + " aplicado para jogadores em um raio de " + numero + " blocos");
                    return true;
                }
            }
        }
